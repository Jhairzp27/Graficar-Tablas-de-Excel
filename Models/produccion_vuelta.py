...
def generar_grafico_produccion_vuelta(filepath, tipo_unidad, num_unidad, ax=None):
    # Leer los datos desde el archivo Excel
    data = pd.read_excel(filepath, sheet_name='Producción')
    rango_filas = None
    columna_produccion_vuelta = None
    columna_promedio_vuelta = None

    # Determinar los rangos de filas y columnas para unidades Grandes y Micros
    if tipo_unidad == "Grandes":
        rango_filas = (1, 27)
        columna_produccion_vuelta = 3  # Columna D
        columna_promedio_vuelta = 4   # Columna E
    ...
    if num_unidad is not None:
        ...
        provision = data.iloc[fila_unidad, columna_produccion_vuelta]
        promedio = data.iloc[fila_unidad, columna_promedio_vuelta]
        ...
        categorias = ['Promedio/Vuelta','Producción/Vuelta']
        ...
        ax.set_title(f'Producción Por Vuelta Anual - Unidad {num_unidad} [$]')
